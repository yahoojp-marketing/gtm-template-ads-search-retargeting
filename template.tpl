...
[
  ...
  {
    "help": "カスタムキーを設定することで、より精緻な条件でサイトリタゲーティングが行えます。例えばkeyにmember_rank、valueにpremiumを指定することで、サイト訪問者の会員ランクがプレミアムであるというデータが蓄積されます。",
    "displayName": "カスタムキー",
    "name": "custom_key",
    "simpleTableColumns": [
      {
...
const onSuccess = () => {
  log('Conductrics: Script loaded successfully.');  
	...
	let params = {};
	if(data.custom_key){
	    for (let i = 0; i < data.custom_key.length; i++){
	      let i_key = data.custom_key[i].key ;
	      let i_value = data.custom_key[i].value ;
	      params[i_key] = i_value ;
	    }
	}
  ...
  ytag({
      ...
      config:{
        yahoo_ss_retargeting_id: data.retargeting_id,
        yahoo_sstag_custom_params: params,
      }
  });
